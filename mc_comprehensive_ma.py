#!/usr/bin/env python3
"""
Estimator Comparison under MA(1) Short-Run Dynamics

Monte Carlo comparison of local Whittle estimators with ARFIMA(0,d,1) data.
This is a companion to mc_comprehensive.py, which uses AR(1) short-run dynamics.
The design, estimators, parameter grid, bandwidth, and seed scheme match
mc_comprehensive.py exactly, but the AR(1) coefficient rho is replaced by the
MA(1) coefficient theta. By construction the theta=0 panel reproduces the
rho=0 panel of the AR(1) table.
"""
...
# Settings (match mc_comprehensive.py, with theta in place of rho)
n_obs = 500
d_list = [-2.2, -1.8, -1.2, -0.6, -0.3, 0.0, 0.3, 0.6, 1.2, 1.8, 2.2]
theta_list = [0.0, 0.5, 0.8]
mc_reps = 10000
alpha = 0.65
m = int(n_obs**alpha)
bounds = (-4.0, 4.0)
...
def run_single_rep(args):
    ...
    d_true, theta, rep_id, seed = args

    # Generate ARFIMA(0,d,1) data with MA(1) short-run dynamics
    x = arfima_arma(n_obs, d_true, phi=0.0, theta=theta, sigma=1.0, seed=seed, burnin=0)
...
def main():
    ...
    print("Estimator Comparison under MA(1) Short-Run Dynamics")
    print("===================================================")
    ...
    for theta in theta_list:
        for d_true in d_list:
            for rep in range(mc_reps):
                seed = (seed_base +
                        theta_list.index(theta) * len(d_list) * mc_reps +
                        d_list.index(d_true) * mc_reps +
                        rep)
                tasks.append((d_true, theta, rep, seed))
    ...
    results_list = []
    for (d_true, theta), estimates_dict in organized_results.items():
    ...
    bias_pivot = results_df.pivot_table(values='bias', index=['d', 'theta'], columns='estimator').round(4)
    mse_pivot = results_df.pivot_table(values='mse', index=['d', 'theta'], columns='estimator').round(4)

    # Create dataframe with proper ordering (theta first, then d)
    combined_results = []
    for theta_val in theta_list:
        for d_val in d_list:
            row_data = {'d': d_val, 'theta': theta_val}

            # Add bias columns
            for est in estimator_names:
                if est in bias_pivot.columns:
                    row_data[f'{est}_bias'] = bias_pivot.loc[(d_val, theta_val), est]
                else:
            ...
            for est in estimator_names:
                if est in mse_pivot.columns:
                    row_data[f'{est}_mse'] = mse_pivot.loc[(d_val, theta_val), est]
                else:
    ...
    column_order = ['d', 'theta'] + bias_cols + mse_cols
    ...
    # Header row
    headers = ['d', 'theta'] + [f'{est}_b' for est in estimator_names] + [f'{est}_mse' for est in estimator_names]
    ...
    for _, row in rounded_df.iterrows():
        values = [f"{row['d']:>8.1f}", f"{row['theta']:>8.1f}"]
        for est in estimator_names:
...
$d$ & $\\theta$ & LW & V & HC & ELW & 2ELW & LWLFC & LW & V & HC & ELW & 2ELW & LWLFC \\\\
\\midrule
"""
    ...
    for theta_val in theta_list:
        ...
        for d_val in d_list:
            ...
            latex_table += f"{d_val:4.1f} & {theta_val:3.1f} "
...
\\begin{{tablenotes}}
\\footnotesize
\\item Notes: Monte Carlo results for $\\ARFIMA(0,d,1)$ processes with $n={n_obs}$, $m={m}$, {mc_reps:,} replications.
    Shaded cells indicate $\\text{{MSE}} > {MSE_THRESHOLD:.2f}$.
\\item LW = Local Whittle, V = Velasco (Kolmogorov), HC = Hurvich--Chen, ELW = Exact Local Whittle, 2ELW = Two-step ELW, LWLFC = Local Whittle robust to low-frequency contamination.
\\end{{tablenotes}}
\\end{{threeparttable}}
\\end{{table}}
"""
    ...
    # Save LaTeX table
    with open('tables/mc_comprehensive_ma1.tex', 'w') as f:
        f.write(latex_table)

    print("LaTeX table saved to: tables/mc_comprehensive_ma1.tex")
