...
\\toprule
 & \\multicolumn{{5}}{{c|}}{{Baseline MSE ($\\mu = {mu_zero}$)}} & \\multicolumn{{5}}{{c}}{{MSE Ratio ($\\mu = {mu_pos}$ / $\\mu = {mu_zero}$)}} \\\\
...
\\item LW = Local Whittle, V = Velasco (Kolmogorov), HC = Hurvich-Chen, ELW = Exact Local Whittle, 2ELW = Two-step ELW with adaptive mean estimation applied to original series.
\\end{{tablenotes}}
\\end{{threeparttable}}
\\end{{table}}
"""
